...
design_app_D = [u(kk,:), w(kk)]; %optimal design 
d00_D = design_app_D(:, 1:end-1); % support points
w00_D = design_app_D(:, end); % optimal weight
L00_D = 1/(det(M))^(1/q);
cvx_status

FIM_D_cvx = zeros(q,q);
for j = 1:length(d00_D)
  xx = [1, d00_D(j, :)]';
  FIM_D_cvx = FIM_D_cvx + w00_D(j) * (xx * xx');
end
...
cvx_end
kk = find(w > tol); % because the computer does not have exact zero
design_app_A = [u(kk,:), w(kk)]; %optimal design 
d00_A = design_app_A(:, 1:end-1); % support points
w00_A = design_app_A(:, end); % optimal weight
L00_A = trace(inv(M)); % optimal objective value 
cvx_status

FIM_A_cvx = zeros(q,q);
for j = 1:length(d00_A)
  xx = [1, d00_A(j, :)]';
  FIM_A_cvx = FIM_A_cvx + w00_A(j) * (xx * xx');
end
...
cvx_precision best
  ...
  tau >= 0;
  -log_det(M) - q*log(tau) <= -log_det(FIM_D_cvx);
  trace_inv(M) - tau*trace_inv(FIM_A_cvx) <=0;
  ones(N, 1)' * w == 1;
  -w <= zeros(N, 1);
cvx_end
kk = find(w > tol); % because the computer does not have exact zero
...
for pig = 1:size(n,1)
  ...
  for ell=1:Nsim 
    ...
    % eff_D = exp(-log(det(FIM)^(1/q)))/exp(L00_D);
    % eff_A = L00_A/trace(inv(FIM));
    eff_D = L00_D/(det(FIM)^(1/q));
    eff_A = L00_A/trace(inv(FIM));
    tau = -min(eff_D, eff_A);
    loss(1) = tau;
...
% calculate the FIM and objective function value for this exact design
  ...
  for j = 1:length(w0)
    ...
    eff_D = L00_D/(det(FIM1)^(1/q));
    eff_A = L00_A/trace(inv(FIM1));
    tau = -min(eff_D, eff_A);
   L0 = -tau;
   loss(2) = tau;
    num_iters = 2;
  ...
  while(T > Tmin && abs(L_prev - L0) > tol_annealing )
    ...
    for h = 1:Nt
      ...
      for j = 1:length(wi)
        xx = [1, di(j, :)]';
        FIMi = FIMi +(xx * xx') * wi(j);
      end
     eff_D = L00_D/(det(FIMi)^(1/q));
      eff_A = L00_A/trace(inv(FIMi));
      tau = -min(eff_D, eff_A);
       Li = tau;
