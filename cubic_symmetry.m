...
% Special treatment at x=0, the weight needs to be divided by half
...
for j=1:size(design_true,2)   
  ...
  w = design_true(2,j);
  fx = power(d, 0:p)';
  FIM_true = FIM_true + (fx * fx') * w;
end
exp(-log(det(FIM_true)^(1/q)))
% [loss1(end),sum(w0_full), sum(n_count)]
